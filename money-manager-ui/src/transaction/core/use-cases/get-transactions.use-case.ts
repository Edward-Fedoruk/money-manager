...
import { IGetTransactionsByDateRangePresenter } from "./get-transactions-presenter.interface";
    ...
    async execute(
        ...
        try {
            const transactions: Transaction[] =
                await this.repository.getTransactionsByDateRange(
            ...
            const incomeTransactions = filterByType(
                transactions,
                "income",
            ) as Array<Transaction & { type: "income" }>;
            const incomeSum = sumTransactions(incomeTransactions);

            const expenseTransactions = filterByType(
                transactions,
                "expense",
            ) as Array<Transaction & { type: "expense" }>;
